...
class ContractDeleteValidationTest extends BaseTest {
    ...
    void testDeleteArchivedContractShouldFail() throws Exception {
        MvcResult mvcResult = this.requestGet(DELETE_PATH, archivedContract.getId())
    ...
    void testDeleteContractWithPaymentPlanShouldFail() throws Exception {
        ...
        MvcResult mvcResult = this.requestGet(DELETE_PATH, contractWithPaymentPlan.getId())
                .andExpect(status().is4xxClientError())
                .andReturn();
    ...
    void cleanupTestData() {
        if (paymentPlan != null && contractPaymentPlanMapper.selectByPrimaryKey(paymentPlan.getId()) != null) {
            contractPaymentPlanMapper.deleteByPrimaryKey(paymentPlan.getId());
        }
        if (normalContract != null && contractMapper.selectByPrimaryKey(normalContract.getId()) != null) {
            contractMapper.deleteByPrimaryKey(normalContract.getId());
        }
        if (archivedContract != null && contractMapper.selectByPrimaryKey(archivedContract.getId()) != null) {
            contractMapper.deleteByPrimaryKey(archivedContract.getId());
        }
        if (contractWithPaymentPlan != null && contractMapper.selectByPrimaryKey(contractWithPaymentPlan.getId()) != null) {
