...
class ContractPaymentPlanControllerTests extends BaseTest {
    ...
    void testAddPlanAmountExceedContractAmount() throws Exception {
        ...
        Assertions.assertEquals(HttpStatus.INTERNAL_SERVER_ERROR.value(), mvcResult2.getResponse().getStatus());
        ...
        Assertions.assertTrue(message.contains("回款总额超出合同金额") || message.contains("exceeds contract amount"));
    ...
    void testUpdatePlanAmountExceedContractAmount() throws Exception {
        ...
        Assertions.assertEquals(HttpStatus.INTERNAL_SERVER_ERROR.value(), mvcResult3.getResponse().getStatus());
    ...
    @Test
    @Order(14)
    void testPlanAmountMustBePositive() throws Exception {
        ContractPaymentPlanAddRequest addRequest = new ContractPaymentPlanAddRequest();
        ...
        Map<String, Object> response = JSON.parseMap(responseContent);
        String message = (String) response.get("message");
        Assertions.assertTrue(message.contains("单期回款金额必须大于0") || message.contains("greater than 0"));
    }
}